...
# === YOUR CODE HERE ===========================================================
      ...
      #   | x1 x2 x3 x4 |
      #   | y1 y2 y3 y4 |
      x = np.array([0,0,nx,nx])*TILE_LENGTH
      y = np.array([0,ny,ny,0])*TILE_LENGTH
      xy = np.array([[0,0,nx,nx],
        [0,ny,ny,0]])*TILE_LENGTH
      print("xy =")
      print(xy)
      A = np.array([
        [xy[0][0],xy[1][0],1,0,0,0,-1*uv[0][0]*xy[0][0],-1*uv[0][0]*xy[1][0]],
        [0,0,0,xy[0][0],xy[1][0],1,-1*uv[1][0]*xy[0][0],-1*uv[1][0]*xy[1][0]],
        [xy[0][1],xy[1][1],1,0,0,0,-1*uv[0][1]*xy[0][1],-1*uv[0][1]*xy[1][1]],
        [0,0,0,xy[0][1],xy[1][1],1,-1*uv[1][1]*xy[0][1],-1*uv[1][1]*xy[1][1]],
        [xy[0][2],xy[1][2],1,0,0,0,-1*uv[0][2]*xy[0][2],-1*uv[0][2]*xy[1][2]],
        [0,0,0,xy[0][2],xy[1][2],1,-1*uv[1][2]*xy[0][2],-1*uv[1][2]*xy[1][2]],
        [xy[0][3],xy[1][3],1,0,0,0,-1*uv[0][3]*xy[0][3],-1*uv[0][3]*xy[1][3]],
        [0,0,0,xy[0][3],xy[1][3],1,-1*uv[1][3]*xy[0][3],-1*uv[1][3]*xy[1][3]]])
      b = np.array([[uv[0][0]],
        [uv[1][0]],
        [uv[0][1]],
        [uv[1][1]],
      ...
      uv_l = np.vstack((uv_l,np.array([1,1])))
      print("uv_l =")
      print(uv_l)


      prod = np.divide(np.dot(Q,uv_l),alpha)
      print('prod_both =')
      print(prod)
      ...
      print('distance between 5th and 6th click =')
      print(distance)
